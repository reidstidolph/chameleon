...
//Set up the response object by calling a function.
    ...
    if(authType==wsc.AUTHTYPE.SERVICE){
        ...
        if (chameleon.configuration.natConfig){
            var config = chameleon.configuration.natConfig;
            authInfo = config;
        }
    ...
    function getSipAuth(authHeaders) {
        ...
        function localAuthGenerator(authHeaders){
            if (authHeaders) {
                chameleon.log("WSC server must be 7.0 or older...generating my own response.");
                ...
                chameleon.log("WSC server must be 7.1 or new...letting SDK generate response.");
                return {"username": chameleon.configuration.sipUsername, "password": chameleon.configuration.sipPassword};
            }
        }
    }
 };
