...
//set up the auth handler
        authHandler.refresh =  function(authType, authHeaders) {
            chameleon.log("authType= " + JSON.stringify(authType) + ", authHeaders= " + JSON.stringify(authHeaders));
...
// Set up the Incoming call handler
            chameleon.wscCallPackage.onIncomingCall = function(callObj, callConfig){
                ...
                chameleon.log("incomingCallCallback provided is:  " + typeof chameleon.configuration.callbacks.incomingCallCallback);
                if (typeof chameleon.configuration.callbacks.incomingCallCallback === "function") {
                    chameleon.log("got a incoming call callback...executing it.");
                    chameleon.configuration.callbacks.incomingCallCallback();
                }
...
/**===============auth Handler============== */
 chameleon.handlers.challengeHandler = function(authType, authHeaders){
    ...
    chameleon.log("Challenge headers: " + JSON.stringify(authHeaders));
    chameleon.log(chameleon.wscSessionObj.sessionState);
...
//Set up the response object by calling a function.
    ...
    if(authType==wsc.AUTHTYPE.SERVICE){
        if (authHeaders === null) {
            chameleon.log("authHeaders not returned...return simple user/pass.")
            authInfo = {'username' : chameleon.configuration.sipUsername, 'password' : chameleon.configuration.sipPassword };
        }
        else {
            chameleon.log("authHeaders returned...generate response.")
   ...
   "iceServers":[
    ...
    function getSipAuth(authHeaders) {
        ...
        function localAuthGenerator(authHeaders){
            ...
            chameleon.log(JSON.stringify(authHeaders));
...
chameleon.handlers.callStateChange = function(callObj, newCallState){
    ...
    // Set up handler for ICE state changes
        pc.oniceconnectionstatechange = function(event) {
            var thisPc = event.target;
            chameleon.log("Ice state change! New state is: " + thisPc.iceConnectionState);
            chameleon.log(event);
            switch(thisPc.iceConnectionState) {
                case "new" :
                    callObj.mediaPath = "NEW";
                    break;
                case "checking" :
            ...
            chameleon.log("getting stats...");
            
            chameleon.log(thisPc.getLocalStreams());
            chameleon.log(thisPc.getLocalStreams()[0].getAudioTracks());
            
            thisPc.getStats(function(e){chameleon.log(JSON.stringify(e));},thisPc.getLocalStreams()[0].getAudioTracks()[0],function(){});
