...
$(document).ready(function() {
	...
	$(".horizSlider").hrzAccordion({
			containerClass     : "container",
			listItemClass      : "listItem",					
			contentWrapper     : "contentWrapper",
			contentInnerWrapper: "contentInnerWrapper",
			handleClass        : "tab",
			handleClassOver    : "tabOver",
			handleClassSelected: "tabSelected"
	});
 });
	   ...
	   setOnEvent: function(i, container, finalWidth, settings){
			$("#"+container+"Handle"+i).bind(settings.eventTrigger,function() {	
						...
						$('[rel='+container+'ContainerSelected]').data('status',1);
							...
							$('[rel='+container+'ContainerSelected]').animate({width: "0px",opacity:"1"}, { queue:true, duration:settings.closeSpeed ,easing:settings.closeEaseAction,complete: function(){	
		...
		if(options){
			...
			return this.each(function(a){    		
				...
				if(settings.fixedWidth){
					finalWidth = settings.fixedWidth + 17;
				}else{
					finalWidth = containerWidth-(elementCount*handleWidth)-handleWidth + 17;
				}
